---
title: "Predicting severity of car incident"
author: "Takeshi Oda"
date: "2020/5/4"
output:
...
**How much would be the severity of the car accident when one car accident happens? **  

This dataset provides response variable Severity i.e., a number between 1 and 4, where 1 indicates the least impact on traffic (short delay) and 4 indicates a significant impact on traffic. (long delay)   
**I defined binary classification variable Severity_bin i.e., "Low" for Seveirty 1 and 2, "High" for Severity 3 and 4 and built a predictive model to estimate probability to have "High" severity against specific input of a street such as weather, time and road structure.**  
...
**To narrow the scope of the model, I extracted accident data of Illinois state in 2019 from original dataset. **
To enrich predictive variable, I downloaded population data for each county in Illinois from https://www.illinois-demographics.com/counties_by_population and combined it with car accident data set.

## 2. Data load and cleaning

To prepare dataset for data mining, I performed following data cleanings.
...
|   Visibility  
|   Window Speed  
| **Only one observations lacled the period of day (i.e. day or night) in sunrise/sunset. civil twilight, nautical twilight and astronomical twilight.I added 'Day' in following four variables.**    
|   Sunrise_Sunset  
|   Civil_Twilight  
|   Nautical_Twilight  
|   Astronomical_Twilight  
| **Following varables are abandoned because proportio of missing value is high.**  
|   Wind_Chill.F.  
|   Precipitation.in.  
...
This dataset contains several continuous variables. To reduce prediction variance from outlier, I applied binning against following variables.  

**Weather Condition**  
50 different weather conditions are categorized into 11 larger categories.  

**Starting Time**  
Each time of the accident is classified into Four categories: Morning, Afternoon, Evening and Night.  
...
**Population**
Cook county has extremly large population 5,223,719 followed by Dupage county's 931,743.
Population of all counties except for Cook county is classified into four bins. Cook county is classified into fifth bin.
...
To find any patterns in correlation of Severity with predictive variables, 
data visualization is performed.

### Key Findings

**Serious accident is most likely to happen in the afternoon while light accident is much more likely to happen in the monring.**   

```{r exploratory_main_1 }
#Severity and Time
...
**Proportion of non traffic signal is much higher in serious accident than light accident**

```{r exploratory_main_3 }
...
```{r exploratory_main_4 }
#Severity and Population
gf_bar(~Severity_bin, fill=~Pop_bin, position = position_dodge(), data=acc_il, main="Severity and Population")
```
**Accidents are frequent in right side of the street. But serious accident is much more frequent in right side than light accident.**
```{r exploratory_main_5 }

gf_bar(~Severity_bin, fill=~Side, position = position_dodge(), data=acc_il, main="Severity and Side")
```


```{r exploratory_other, echo =FALSE, eval=FALSE }

##### This section include data visualization using original four levels of Severity ##### 


#Numerical variables

#Severity and temperature
gf_boxplot(Temperature.F.~Severity, data=acc_il)

#Severity and Humidity
gf_boxplot(Humidity...~Severity, data=acc_il)


#Severity and Pressure
gf_boxplot(Pressure.in.~Severity, data=acc_il)

#Severity and Visibility
gf_boxplot(Visibility.mi.~Severity, data=acc_il)

#Severity and Window Speed
gf_boxplot(Wind_Speed.mph.~Severity, data=acc_il)

#Categorical variables

#Severity and day of week
mtx1 <- table(acc_il$weekday, acc_il$Severity)
heatmap(mtx1, xlab="Sevirity", Colv=NA, Rowv=NA, cexRow = 0.5, cexCol = 0.5)

#Severity and Weather condition
mtx2 <- table(acc_il$Weather_Condition, acc_il$Severity)
heatmap(mtx2, xlab="Sevirity", Colv=NA, Rowv=NA, cexRow = 0.5, cexCol = 0.5)

#Severity and Time
gf_bar(~Severity, fill=~time_bin, position = position_dodge(), data=acc_il)

#Severity and Population
gf_bar(~Severity, fill=~Pop_bin, position = position_dodge(), data=acc_il)


#Other categorical variables
gf_bar(~Severity, fill=~Side, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Amenity, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Bump, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Crossing, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Give_Way, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Junction, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~No_Exit, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Railway, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Roundabout, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Station, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Stop, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Traffic_Calming, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Traffic_Signal, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Turning_Loop, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Sunrise_Sunset, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Civil_Twilight, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Nautical_Twilight, position = position_dodge(), data=acc_il)
gf_bar(~Severity, fill=~Astronomical_Twilight, position = position_dodge(), data=acc_il)
```

## 4. Model Selection

### Candidates algorithms
In this project, I took an approach to **balance between predictive accuracy and model interpretation.** While the model should provide high capability to predict serious car accident, it should enable local government to gain insight into hidden factors of serious car accidents. Therefore, I chose three classification methods, **Logistics Regression, Bagging and Random Forest**. With Logistics Regression, log of odds ratio of being serious car accident is modeled as linear combination of each variable. Bagging and Random Forest allow us to  quantify importance of predictive variables through Variable Importance plot.  
...
To select best model among three algorithms, cross validation was performed on training data set. **Training data set is split into four folds and model fitting and prediction are performed for each fold**. Model which achieves least error rate across four folds is selected as best model.



### Best Model
| **Random Forest** achieved least **cross validation error rate 0.22** restricting **9 variables** per tree.  
| This means the model explain around 78 percent of variation of severity.   
| From variable importance plot, following five variables are identified as strong predictor.  
|  Time
...
```
## 5. Model Validation
As a final step of predictive modeling, prediction error rate is measured.
Random forest model with number of variable = 9 is fitted on all the training data set and prediction is performed on validation set.
